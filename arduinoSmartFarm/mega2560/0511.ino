...
int BUZZER_State = LOW;                       //D13 Buzzer &  sound
...
if((unsigned long)(pumpCurrentMillis - pumpPreMillis) >= pumpInterval){
  if(pumpState){
    pumpInterval = pumpOffMin;
    Serial.println(pumpState);
    Serial.println("펌프아웃");
  }
  else{
    pumpInterval = pumpOnMin;
    Serial.println("펌프 동작");
  }
  pumpState = !(pumpState);
...
if((unsigned long)(OxyCurrentMillis - OxyPreMillis) >= OxyInterval){
  if(OxyState){
