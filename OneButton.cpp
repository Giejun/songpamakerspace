...
void OneButton::tick(bool activeLevel){
  ...
  if(_state == 0) { // waiting for menu pin being pressed.
    ...
    else if((activeLevel) && ((unsigned long)(now - _startTime) > _pressTicks)){
    ...
    if((_doubleClickFunc == NULL && _paramDoubleClickFunc == NULL) || (unsigned long)(now - _startTime) > _clickTicks){   // this was only a single short click
      if(_clickFunc)
        _clickFunc();
      if(_paramClickFunc)
    ...
    else if((activeLevel) &&  ((unsigned long)(now - _stopTime) > _debounceTicks)){
